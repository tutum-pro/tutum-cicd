...
ansible localhost -m include_role -a name=tutum_pro.cicd.k8s.tutum_manifests \
...
# Generate from existing inventory file
ansible localhost -m include_role -a name=tutum_pro.cicd.k8s.tutum_manifests \
  -e k8s_tutum_manifests_output_path=./manifests \
  -e k8s_tutum_manifests_inventory_path=./inventory/k8s.ini
```
...
| Variable | Default | Description |
|----------|---------|-------------|
| `k8s_tutum_manifests_inventory_path` | `""` | Path to existing inventory file to import configuration |

When `k8s_tutum_manifests_inventory_path` is set, the role reads configuration from the specified inventory file (e.g., one generated by `tutum_init`). This allows generating manifests with the same configuration used for deployment.
